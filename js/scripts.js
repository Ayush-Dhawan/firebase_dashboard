koimport React, {useEffect, useState} from 'react';
...
const TransactionsPage = ({
  ...
  useEffect(() => {
    if (startDate && endDate) {
      ...
      const filtered = transactions.filter((txn) => {
        const txnDate = new Date(txn.creationDate);
        const txnDateString = txnDate.toISOString().split("T")[0];
        return txnDateString >= startDate && txnDateString <= endDate;
      });
          ...
          <div className="col-2 justify-content-center">
            <div className="card">
              <img
                ...
                src={`${process.env.PUBLIC_URL}/images/${customer.username}.png`}
                alt={customer.name}
                onError={(e) => {
                  e.target.src = Placeholder(options);
                }}
            ...
            <div className="justify-content-center jumbotron-intro mb-4 pb-5 pt-4 pt-5">
              <div className="card w-50 mx-auto mt-3">
                <div className="card-body">
                  <div className="card-content">
                    <div className="card-title text-center">
                      <h1>{customer.name}'s Transaction History</h1>
                    </div>
                    <p>
                      See how you've earned your {customer.points.toLocaleString()} points - sort
                      and filter to see your data as needed!
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="col-12">
            <div className="card">
              ...
              >
                ...
                <Button
                  color={startDate && endDate ? "danger" : "primary"}
                  className="ms-3"
                  onClick={startDate && endDate ? clearFilter : applyFilter}
                >
                  {startDate && endDate ? "Clear Filter" : "Apply Filter"}
                </Button>
              </div>
